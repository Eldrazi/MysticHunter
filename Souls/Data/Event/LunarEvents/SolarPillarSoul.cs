...
{
	public class SolarPillarSoul : PostHMSoul, IEventSoul
	{
		public override short soulNPC => NPCID.LunarTowerSolar;
		public override string soulDescription => "Summons a Melee boosting mini Solar Pillar.";
		...
		public override bool SoulUpdate(Player p, short stack)
		{
			...
			{
				if (Main.projectile[i].active && Main.projectile[i].owner == p.whoAmI && Main.projectile[i].type == ModContent.ProjectileType<SolarPillarSoul_Proj>())
				{
			...
			Projectile.NewProjectile(p.Center, default, ModContent.ProjectileType<SolarPillarSoul_Proj>(), 200, 0, p.whoAmI, stack);

			return (true);
		}
	}

	public class SolarPillarSoul_Proj : ModProjectile
	{
		...
		public override bool PreAI()
		{
			...
			if (owner.active && !owner.dead && sp.BlueSoulNet.soulNPC == NPCID.LunarTowerSolar)
			{
				projectile.timeLeft = 2;

				// Boost owner's ranged stats.
				owner.statDefense += 20 + 5 * (int)projectile.ai[0];
				owner.meleeDamage += 0.2f + (0.05f * projectile.ai[0]);
			}
				...
				if (owner.whoAmI == Main.myPlayer)
				{
					...
					for (int i = 0; i < Main.maxNPCs && targetsAmount < 3; ++i)
					{
						...
						Projectile.NewProjectileDirect(projectile.Center, Vector2.Normalize(directionTowardsNPC) * 12,
							ProjectileID.Daybreak, projectile.damage, 1f, projectile.owner);
					}
		...
		private void DustEffect()
		{
			for (int i = 0; i < 15; i++)
			{
				Dust d = Main.dust[Dust.NewDust(projectile.position, projectile.width, projectile.height, DustID.SolarFlare, -projectile.velocity.X * .2f, -projectile.velocity.Y * .2f, 100, default, 2f)];
				d.noGravity = true;
			}
		}
