...
{
	public class EyezorSoul : PostHMSoul, IEventSoul
	{
		public override short soulNPC => NPCID.Eyezor;
		public override string soulDescription => "Summons a shambling Eyezor that shoots a regular laser.";
		...
		public override bool SoulUpdate(Player p, short stack)
		{
			...
			for (int i = 0; i < amount; ++i)
			{
				...
				Projectile proj = Main.projectile[Projectile.NewProjectile(spawnPos, Vector2.Zero, ModContent.ProjectileType<Eyezor_Proj>(), damage, 0, p.whoAmI)];
	...
	public class Eyezor_Proj : ModProjectile
	{
		public override string Texture => "Terraria/NPC_" + NPCID.Eyezor;

		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Eyezor");
			Main.projFrames[projectile.type] = Main.npcFrameCount[NPCID.Eyezor];
		}
					...
					if (Main.myPlayer == projectile.owner)
					{
						for (int i = 0; i < Main.maxNPCs; ++i)
						{
							if (Main.npc[i].active && Main.npc[i].CanBeChasedBy(projectile) &&
								Vector2.Distance(projectile.Center, Main.npc[i].Center) <= 600 && Collision.CanHitLine(projectile.Center, 1 ,1, Main.npc[i].Center, 1, 1))
							{
								Vector2 direction = Vector2.Normalize(Main.npc[i].Center - projectile.Center);
								int newProj = Projectile.NewProjectile(projectile.Center, direction * 8f, ProjectileID.RayGunnerLaser, projectile.damage, 0.2f, projectile.owner);
