...
{
	public class CrystalThresherSoul : PostHMSoul, IEventSoul
	{
		public override short soulNPC => NPCID.SandsharkHallow;
		public override string soulDescription => "Send out a charging Crystal Thresher.";
		...
		public override bool SoulUpdate(Player p, short stack)
		{
			...
			Projectile.NewProjectile(p.Center, velocity, ModContent.ProjectileType<CrystalThresherSoulProj>(), damage, knockBack, p.whoAmI, 0, (stack >= 9 ? 1 : 0));
			return (true);
		}
	}

	internal sealed class CrystalThresherSoulProj : ModProjectile
	{
		public override string Texture => "Terraria/Images/NPC_" + NPCID.SandsharkHallow;
		...
		public override void Kill(int timeLeft)
		{
			...
			{
				Dust d = Main.dust[Dust.NewDust(projectile.position, projectile.width, projectile.height, DustID.PinkCrystalShard, -projectile.velocity.X * .2f, -projectile.velocity.Y * .2f, 100)];
				d.noGravity = true;
			}
		}
		...
		public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
		{
			for (int i = 0; i < 10; ++i)
			{
				Dust.NewDust(target.position, target.width, target.height, DustID.PinkCrystalShard, projectile.velocity.X * .2f, projectile.velocity.Y * .2f, 100);
			}
		}
	}
}
