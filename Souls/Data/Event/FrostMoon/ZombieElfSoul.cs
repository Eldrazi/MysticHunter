...
{
	public class ZombieElfSoul : PostHMSoul, IEventSoul
	{
		public override short soulNPC => NPCID.ZombieElf;
		public override string soulDescription => "Summons a lumbering Zombie Elf.";
		...
		public override bool SoulUpdate(Player p, short stack)
		{
			...
			for (int i = 0; i < amount; ++i)
			{
				...
				if (amount == 2)
					spawnPos += new Vector2(-45 * (i == 0 ? 1 : -1), 0);
				else if (amount == 3)
					spawnPos += new Vector2(-45 * (1 - i), 0);

				Projectile proj = Main.projectile[Projectile.NewProjectile(spawnPos, Vector2.Zero, ModContent.ProjectileType<ZombieElfSoulProj>(), damage, 0, p.whoAmI)];
	...
	public class ZombieElfSoulProj : ModProjectile
	{
		...
		{
			DisplayName.SetDefault("Zombie Elf");
		...
		public override bool PreAI()
		{
			...
			else if (projectile.ai[0] == 1)
			{
				...
				projectile.velocity.X = projectile.direction * 1.5f;
