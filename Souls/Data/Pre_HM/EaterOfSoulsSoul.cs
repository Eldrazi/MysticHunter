	...
	public class EaterOfSoulsSoul : PreHMSoul
	{
		...
		public override bool SoulUpdate(Player p, short stack)
		{
			...
			for (int i = 0; i < Main.maxProjectiles; ++i)
				if (Main.projectile[i].active && Main.projectile[i].owner == p.whoAmI && Main.projectile[i].type == ModContent.ProjectileType<EaterOfSoulsSoulProj>())
					Main.projectile[i].Kill();

			Projectile.NewProjectile(p.Center, Vector2.Zero, ModContent.ProjectileType<EaterOfSoulsSoulProj>(), 20 + stack, .1f + .02f * stack, p.whoAmI);
			return (true);
		}
