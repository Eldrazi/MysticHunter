...
namespace MysticHunter.Souls.Data.Pre_HM
{
	public class DevourerSoul : PreHMSoul
	{
		...
		public override bool SoulUpdate(Player p, short stack)
		{
			...
			{
				if (Main.projectile[i].active && Main.projectile[i].owner == p.whoAmI && Main.projectile[i].type == ModContent.ProjectileType<DevourerSoulProj>())
					Main.projectile[i].Kill();
			}

			Projectile.NewProjectile(p.Center, Vector2.Zero, ModContent.ProjectileType<DevourerSoulProj>(), 20 + (2 * stack), .1f, p.whoAmI, -1);
			return (true);
		}
	}
	...
	public class DevourerSoulProj : ModProjectile
	{
		...
		public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
		{
			...
			Texture2D tailPartTex = ModContent.GetTexture("MysticHunter/Souls/Data/Pre_HM/DevourerSoulProj_Chain").Value;
