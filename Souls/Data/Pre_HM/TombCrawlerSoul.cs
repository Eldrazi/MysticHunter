...
{
	public class TombCrawlerSoul : ISoul
	{
		public bool acquired { get; set; }

		public short soulNPC => NPCID.TombCrawlerHead;
		public string soulDescription => "Grow a spiny tail.";
		...
		public bool SoulUpdate(Player p, short stack)
		{
			...
			{
				if (Main.projectile[i].active && Main.projectile[i].owner == p.whoAmI && Main.projectile[i].type == ProjectileType<TombCrawlerSoulProj>())
					Main.projectile[i].Kill();
			}

			Projectile.NewProjectile(p.Center, Vector2.Zero, ProjectileType<TombCrawlerSoulProj>(), 15 + (2 * stack), .1f, p.whoAmI, -1);
			return (true);
		}
	}
	...
	public class TombCrawlerSoulProj : ModProjectile
	{
		...
		public override bool PreAI()
		{
			...
			if (player.dead || player.GetModPlayer<SoulPlayer>().souls[1] == null || player.GetModPlayer<SoulPlayer>().souls[1].soulNPC != NPCID.TombCrawlerHead)
				projectile.Kill();
		...
		public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
		{
			...
			Texture2D tailPartTex = GetTexture("MysticHunter/Souls/Data/Pre_HM/TombCrawlerSoulProj_Chain");
