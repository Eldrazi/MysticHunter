	...
	public class EnchantedSwordSoul : PostHMSoul
	{
		...
		public override bool SoulUpdate(Player p, short stack)
		{
			for (int i = 0; i < Main.maxProjectiles; ++i)
				if (Main.projectile[i].active && Main.projectile[i].owner == p.whoAmI && Main.projectile[i].type == ModContent.ProjectileType<EnchantedSwordSoulProj>())
			...
			{
				Projectile.NewProjectile(p.Center, velocity, ModContent.ProjectileType<EnchantedSwordSoulProj>(), damage, .2f, p.whoAmI, (int)WeaponSoulState.Idle);
			}
			return (true);
		}
	}
