...
{
	public class EnchantedSwordSoul : PostHMSoul
	{
		public override short soulNPC => NPCID.EnchantedSword;
		public override string soulDescription => "Summons Enchanted Swords.";
		...
		public override bool SoulUpdate(Player p, short stack)
		{
			for (int i = 0; i < Main.maxProjectiles; ++i)
				if (Main.projectile[i].active && Main.projectile[i].owner == p.whoAmI && Main.projectile[i].type == ProjectileType<EnchantedSwordSoulProj>())
			...
			{
				Projectile.NewProjectile(p.Center, velocity, ProjectileType<EnchantedSwordSoulProj>(), damage, .2f, p.whoAmI, (int)WeaponSoulState.Idle);
			}
			return (true);
		}
	}

	public class EnchantedSwordSoulProj : ModProjectile
	{
		/// <summary>
		...
		public override string Texture => "Terraria/NPC_84";
		...
		public override bool PreAI()
		{
			...
			if (owner.dead || owner.GetModPlayer<SoulPlayer>().activeSouls[(int)SoulType.Blue].soulNPC != NPCID.EnchantedSword)
				projectile.Kill();
			...
			else if (AIState == WeaponSoulState.Attacking)
			{
				...
				projectile.rotation = MathHelper.Lerp(projectile.rotation, projectile.velocity.ToRotation() + MathHelper.PiOver4, .1f);
			...
			// Visual effects.
			Lighting.AddLight(projectile.Center, 0.8f, 0.6f, 0.8f);
