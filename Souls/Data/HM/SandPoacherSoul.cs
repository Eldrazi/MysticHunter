...
namespace MysticHunter.Souls.Data.HM
{
	public class SandPoacherSoul : PostHMSoul
	{
		...
		public override bool SoulUpdate(Player p, short stack)
		{
			...
			{
				if (Main.projectile[i].active && Main.projectile[i].owner == p.whoAmI && Main.projectile[i].type == ModContent.ProjectileType<SandPoacherSoulProj>())
					Main.projectile[i].Kill();
			}

			Projectile.NewProjectile(p.Center, Vector2.Zero, ModContent.ProjectileType<SandPoacherSoulProj>(), 70 + (10 * stack), .3f, p.whoAmI, -1);
			return (true);
		}
	}

	public class SandPoacherSoulProj : ModProjectile
	{
		...
		{
			DisplayName.SetDefault("Venomous Tail");
		}
		...
		public override bool PreAI()
		{
			...
			if (owner.active && !owner.dead && owner.GetModPlayer<SoulPlayer>().BlueSoul?.soulNPC == NPCID.DesertScorpionWalk)
			...
			if (projectile.ai[0] == -1)
			{
				...
				if (projectile.ai[1]++ >= 120)
				{
					...
					{
						if (Main.npc[i].active && Vector2.Distance(owner.Center, Main.npc[i].Center) <= TailLength)
						{
							projectile.ai[0] = i;
							projectile.ai[1] = 0;
		...
		public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
		{
			...
			Texture2D tailPartTex = ModContent.GetTexture("MysticHunter/Souls/Data/HM/SandPoacherSoulProj_Chain").Value;
