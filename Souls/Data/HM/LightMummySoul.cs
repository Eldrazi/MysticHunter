...
namespace MysticHunter.Souls.Data.HM
{
	public class LightMummySoul : PostHMSoul
	{
		...
		public override bool SoulUpdate(Player p, short stack)
		{
			...
			for (int i = 0; i < amount; ++i)
			{
				...
				Projectile proj = Main.projectile[Projectile.NewProjectile(spawnPos, Vector2.Zero, ModContent.ProjectileType<LightMummySoulProj>(), damage, 0, p.whoAmI)];
	...
	{
		public override string Texture => "Terraria/Images/NPC_" + NPCID.LightMummy;
		...
		private void ConvertTiles()
		{
			...
			for (int x = minX; x <= maxX; ++x)
			{
				for (int y = minY; y <= maxY; ++y)
				{
					...
					{
						if (Main.tile[x, y].type == TileID.CorruptGrass || Main.tile[x, y].type == TileID.CrimsonGrass || Main.tile[x, y].type == TileID.Grass)
						{
