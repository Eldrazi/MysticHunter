...
namespace MysticHunter.Souls.Data.HM
{
	public class MummySoul : PostHMSoul
	{
		...
		public override bool SoulUpdate(Player p, short stack)
		{
			...
			for (int i = 0; i < amount; ++i)
			{
				...
				Projectile proj = Main.projectile[Projectile.NewProjectile(spawnPos, Vector2.Zero, ProjectileType<MummySoulProj>(), damage, 0, p.whoAmI)];
	...
	{
		public override string Texture => "Terraria/NPC_78";
		...
		private void ConvertTiles()
		{
			...
			for (int x = minX; x <= maxX; ++x)
			{
				for (int y = minY; y <= maxY; ++y)
				{
					...
					{
						if (Main.tile[x, y].type == TileID.CorruptGrass || Main.tile[x, y].type == TileID.FleshGrass || Main.tile[x, y].type == TileID.HallowedGrass)
						{
